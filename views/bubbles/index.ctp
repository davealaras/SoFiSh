<div class="bubbles index">
	<h2><?php __('Bubbles');?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id');?></th>
			<th><?php echo $this->Paginator->sort('content');?></th>
			<th><?php echo $this->Paginator->sort('user_id');?></th>
			<th><?php echo $this->Paginator->sort('sofish_object_id');?></th>
			<th><?php echo $this->Paginator->sort('parent_sofish_object_id');?></th>
			<th><?php echo $this->Paginator->sort('created');?></th>
			<th class="actions"><?php __('Actions');?></th>
	</tr>
	<?php
	$i = 0;
	foreach ($bubbles as $bubble):
		$class = null;
		if ($i++ % 2 == 0) {
			$class = ' class="altrow"';
		}
	?>
	<tr<?php echo $class;?>>
		<td><?php echo $bubble['Bubble']['id']; ?>&nbsp;</td>
		<td><?php echo $bubble['Bubble']['content']; ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($bubble['User']['name'], array('controller' => 'users', 'action' => 'view', $bubble['User']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($bubble['SofishObject']['id'], array('controller' => 'sofish_objects', 'action' => 'view', $bubble['SofishObject']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($bubble['ParentSofishObject']['id'], array('controller' => 'sofish_objects', 'action' => 'view', $bubble['ParentSofishObject']['id'])); ?>
		</td>
		<td><?php echo $bubble['Bubble']['created']; ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View', true), array('action' => 'view', $bubble['Bubble']['id'])); ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $bubble['Bubble']['id'])); ?>
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $bubble['Bubble']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $bubble['Bubble']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
	));
	?>	</p>

	<div class="paging">
		<?php echo $this->Paginator->prev('<< ' . __('previous', true), array(), null, array('class'=>'disabled'));?>
	 | 	<?php echo $this->Paginator->numbers();?>
 |
		<?php echo $this->Paginator->next(__('next', true) . ' >>', array(), null, array('class' => 'disabled'));?>
	</div>
</div>
<div class="actions">
	<h3><?php __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Bubble', true), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Users', true), array('controller' => 'users', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New User', true), array('controller' => 'users', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Sofish Objects', true), array('controller' => 'sofish_objects', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Sofish Object', true), array('controller' => 'sofish_objects', 'action' => 'add')); ?> </li>
	</ul>
</div>